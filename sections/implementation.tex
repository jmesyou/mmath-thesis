...
\begin{figure}[!htb]
	...
	def generateLocal(vdef: ValDef, desc: FrameDescriptor): LocalVal = {
...
\begin{figure}[!htb]
	...
	def parseDefDef(ddef: DefDef): DefDefNode | DefDefTemplate = {		
		val tparams = ddef.params.filter(_.isInstanceOf[TypeDef]).lengt
		if (tparams == 0)
...
\begin{figure}[!htb]
	\begin{minted}{scala}
	def generateLocal(types: Array[Type], defn: ValDef | TypeDef, desc: FrameDescriptor): LocalVal = 
		defn match {
			case tdef: TypeDef => 
...
\begin{figure}[!htb]
	\begin{minted}{scala}
	def specializeClass(cdef: ClassDef, args: Array[Type]): Type = {
		val typeDefs = cdef.body.filter(_.isInstanceOf[TypeDef])
		val specializedValDefs = 
			cdef.body
				.filter(_.isInstanceOf[ValDef])
				.map(rewriteValDef(args))
		val specializedDefDefs = 
			cdef.body
				.filter(_.isInstanceOf[DefDef])
				.map(specializeMethod(args))
		
		cdef.copy(body=typeDefs ++ specializedValDefs ++ specializedDefDefs)
	}
	\end{minted}
	\caption{Pseudocode for steps to specialize a \scalainline{ClassDef}.}
...
Many elements of class specialization will reuse mechanisms we have previously introduced and similarly takes advantage of the high-level information available in TASTy.
...
Similar to how type definitions become parameters when reified in the context of a \scalainline{DefDef}, reified type definitions in the context of \scalainline{ClassDef} become fields.
...
We rewrite both value definitions as well method definitions to transform polymorphic definitions into monomorphic
...
\begin{figure}[!htb]
	...
	class AppliedTypeNode(@Child con: TypeNode, @Children targs: Array[TypeNode]) extends TypeNode {
		...
		override def resolve(frame: VirtualFrame): Type {
			val tpe = con.resolve(frame)
			
			val types = Array.empty[Type]
			for (targ <- targs)
			types += targ.resolve(frame)
			
			specializeClass(tpe, types)
		}
	}
	\end{minted}
\end{figure}

The \scalainline{AppliedType} is the analogue of \scalainline{TypeApply} for type applications when creating a class instance.
...
\begin{figure}[!htb]
	\begin{minted}{scala}
    def rewriteValDef(args: Array[Type])(vdef: ValDef): ValDef = {
    	val idx = indexOf(vdef.tpt.tpe)
    	if (idx == -1)
    		return vdef
    		
    	val tpe = types(idx)
    	vdef.copy(tpt=TypeIdent(tpe.symbol))
	}
...
When combined with type arguments that are resolved dynamically, we rewrite polymorphic value definitions inside a class definition to their specialized monomorphic counterparts.
Similar 

\begin{figure}[!htb]
	\begin{minted}{scala}
	def specializeMethod(args: Array[Type])(ddef: DefDef): DefDef = {
		val specializedParams = ddef.params.map(rewriteValDef(args))
		val specializedRhs = specializeBody(args)(ddef.rhs)
		
		ddef.copy(params=specializedParams, rhs=specializedRhs)
	}
	\end{minted}
\end{figure}

After the specialization of a \scalainline{DefDef}, it is still possible that a \scalainline{DefDef} contains polymorphic semantics.
...
We introduce an example, given in figure \ref{example:list-contains-example}, which will motivate the specialization of storage layouts in shapes.
The example exercises method invocations where the polymorphism is only class-based as well both class-based and method-based.
