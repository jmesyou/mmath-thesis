...
The implementation of our interpreter does not incorporate the same runtime code to handle polymorphic array operations.
Instead, TastyTruffle always intrinsifies array operations and specializes polymorphic array operations so that they are monomorphic array operations.
We defer the discussion of this optimization until the second half of this chapter.
...
Using indirect calls instead of direct calls comes with performance overheads as indirect call nodes cannot be inlined and inhibits Graal's dynamic intraprocedural analyses.
...
Furthermore, this allows the calls site to be inlined, allowing a feedback loop of optimizations to propagate through the inlined tree.
The size of an polymorphic inline cache must be kept reasonable such that the cost of searching the cache should not defeat the speedup afforded by using the cache.
If the size of the cache exceeds the limit set, the call node is rewritten to use an indirect calls as the cost of inline cache lookups will outweight the penalty of an indirect call. 
...
The \scalainline{Block} is unique among term trees as it can describes data as well as code.
...
\end{figure}
We divide the dicussion of blocks into the resolution of local variables when encountering a \scalainline{ValDef} tree and 

\subsubsection*{Generating Frame Slots from \texttt{ValDef} trees} 
...
In order to execute the tree, name binding must be resolved for variables of differing scopes.
...
Recall that a \scalainline{ValDef} contains the simple, unqualified name of the declaration, the type as represented in the source program and the intializer. 
