...
On the other hand, the storage layout of the \scalainline{tail} field does not have to change between instances of \scalainline{Cons[Int]} and \scalainline{Cons[String]} as they both are reference types.
...
Consider the \scalainline{Nil} class given in figure \ref{example:nil-impl}, a simplified TASTy-like equivalent is given in figure \ref{example:decomp-object}
...
\begin{figure}[!htb]
	...
	\end{minted} 
	\caption{Example multiple possible placements of \scalainline{ValDef} nodes in a \scalainline{ClassDef}}
...
For example, the type \scalainline{Cons[T]} would be represented as an applied type tree, where \scalainline{Cons} is the constructor and \scalainline{T} is the type argument.
...
In a polymorphic context in which a type variable is replaced by the reference type \javainline{Object}, an \scalainline{Int} value is not passed directly but by reference to an object of class \javainline{Integer} that contains the primitive value.
...
\begin{figure}[!htb]
	...
	\end{minted}
	\caption{Implementation of \scalainline{execute_generic1} of \scalainline{EqualsNodeGen}}
	\label{impl:node-rewriting-state1}
\end{figure}
...
Figure \ref{impl:node-rewriting-state1} gives the implementation of \scalainline{execute_generic1}.
	...
	private boolean executeAndSpecialize(Object lhs, Object rhs) {
		...
		if (lhs instanceof Integer) {
			...
			if (rhs instanceof Integer) {
				...
				return this.equalsInt(lhs_int, rhs_int);
			}
		}
...
When invocations of a root node exceed a limit, Graal JIT compiles its children trees into native machine code using \textit{partial evaluation}.
...
For example, consider the partial evaluation of a \scalainline{EqualsNodeGen} node in the \scalainline{equalsInt} state.
The \javainline{current_state} field of the node is annotated with the \javainline{CompilationFinal} directive.
Truffle provides the \javainline{CompilationFinal} directive to indicate that a value that may not be a constant in the guest language implementation \textit{will} be a constant when being partially evaluated.
Because the state is a compilation constant, the condition on line $17$ of \ref{impl:node-rewriting-gen} evaluates to \javainline{true} when the state is $1$ (\scalainline{equalsInt}).
As a result, only the code for \javainline{execute_int_int0} (provided in \ref{impl:node-rewriting-state2}) will be compiled.
The generated implementation of \javainline{execute_int0_int0} contains checks for the specialization invariant.
These checks act as points in the control flow of the compiled code to deoptimize if these invariants are violated.
The resulting code supplied to JIT compilation is the specialization of the \scalainline{EqualsNode} for the \scalainline{equalsInt} state.
