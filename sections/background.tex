	...
	ClassDef(
		...
		List(
			...
			DefDef(
				"tail",
				Nil,
				Applied(TypeIdent("List"), List(TypeIdent("T"))),
				None
			),
...
Note that while the abstract \scalainline{List} class did not explicit declare a constructor, the compiler autogenerates and inserts the appropriate constructor implementation.
...
Terms with no children, such as \scalainline{Ident}, retain their types and those types 'flow' upwards to their parent terms.
...
The node declares semantics of the equality operation between Integers and as well as values of type \scalainline{Any}.
The \scalainline{Any} is the super type of every type in Scala, as such, this equality node has semantics for every case.
